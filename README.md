...
**SentinelPrompt** is a production-quality, high-fidelity AI Security Prompt Builder running entirely client-side. It empowers cybersecurity professionals—penetration testers, threat hunters, malware analysts, detection engineers, and security researchers—to design, optimize, compile, and share highly precise system and workflow prompts for modern Large Language Models (LLMs).
...
* **Heuristic Prompt Optimizer**: An on-device analyzer scoring your prompts (0-100%) and producing contextual suggestions (e.g. specifying target technology details, adding rate limits, isolating input variables).
...
* **PWA Enabled**: Installable on desktop & mobile, working fully offline.
...
4. **Verify TypeScript compilation & build distribution package**:
   ```bash
   npm run build
   ```
